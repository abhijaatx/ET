...
import { Bars3Icon, BookmarkIcon, CalendarDaysIcon, HeartIcon } from "@heroicons/react/24/outline";
...
export default function BookmarksPage() {
  ...
  const loadBookmarks = useCallback(
    async (reset = false) => {
      ...
      const res = await fetch(
        `${API_URL}/api/feed/bookmarked?offset=${currentOffset}&limit=20${dateParam}`,
        {
          credentials: "include"
        }
      );
  ...
  const sendSignal = async (openedBriefingOverride?: boolean) => {
    ...
    // If we unbookmarked it, refresh the list
    if (!saved && !openedBriefingOverride) {
      loadBookmarks(true);
    }
  };
          ...
          <header className="sticky top-0 z-10 bg-white/95 backdrop-blur-md border-b border-et-border">
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl font-bold font-serif tracking-tight text-et-headline">Bookmarks</h1>
              </div>
              <div className="flex items-center gap-4">
                {selectedDate && (
                  <button 
          ...
          <div className="max-w-5xl mx-auto px-4 py-8 relative">
            ...
            <div className="space-y-10">
              ...
              <div className="text-center py-20 text-et-secondary">
                ...
                <p>{selectedDate ? `No bookmarks for ${selectedDate.toDateString()}` : "No bookmarks yet."}</p>
              </div>
            ) : (
