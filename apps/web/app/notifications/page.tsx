...
export default function NotificationsPage() {
  ...
  const loadNotifications = useCallback(
    async (reset = false) => {
      ...
      const res = await fetch(
        `${API_URL}/api/notifications?offset=${currentOffset}&limit=20${dateParam}`,
        {
          credentials: "include"
        }
      );
  ...
  const groupedArticles = useMemo(() => {
    ...
    articles.forEach(article => {
      const date = article.publishedAt ? new Date(article.publishedAt).toDateString() : "Unknown";
  ...
  const handleOpen = (article: FeedArticle) => {
    ...
    setScrollDepth(0);
    setShared(false);
    setSaved(article.isBookmarked ?? false);
          ...
          <header className="sticky top-0 z-10 bg-white/95 backdrop-blur-md border-b border-et-border">
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl font-bold font-serif tracking-tight text-et-headline">Notifications</h1>
              </div>
              <div className="flex items-center gap-4">
                {selectedDate && (
                  <button 
            ...
            <div className="space-y-10">
              {articles.length === 0 && !loading ? (
                <div className="text-center py-20 text-et-secondary">
                  ...
                  <p>{selectedDate ? `No updates for ${selectedDate.toDateString()}` : "No notifications yet."}</p>
                </div>
              ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {liked ? (
                ...
                <HeartIconSolid className="w-6 h-6 opacity-20" /> 
              )}
            </button>
